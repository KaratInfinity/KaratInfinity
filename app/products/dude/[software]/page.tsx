...
type PageProps = { params: { software: string } };

export function generateStaticParams() {
  return softwareProducts.map((product) => ({ software: product.slug }));
}

export function generateMetadata({ params }: PageProps): Metadata {
  const product = getSoftwareProduct(params.software);
  if (!product) return { title: 'DUDE' };
  return pageMeta(product.title, product.description, `/products/dude/${product.slug}/`);
}

export default function DudeProductPage({ params }: PageProps) {
  const product = getSoftwareProduct(params.software);
  if (!product) notFound();
    ...
    <main className="page-main" id="main-content">
      <section className="page-hero module-hero">
        ...
        <div className="shell relative z-10 module-hero-content">
          <p className="breadcrumb"><Link href="/">Home</Link><span>/</span><Link href="/products/">Products</Link><span>/</span><Link href="/products/dude/">DUDE</Link><span>/</span><span>{product.shortTitle}</span></p>
        ...
        <div className="shell module-hero-footer"><span>DUDE / {product.shortTitle}</span><a href="#module-overview">Explore capabilities <ArrowRight className="h-4 w-4" /></a></div>
      </section>
          ...
          <div>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {product.capabilities.map((capability, index) => (
                <div key={capability} className="info-card module-feature flex flex-col justify-between">
      ...
      <section className="section-block surface-section">
        <div className="shell">
          ...
          <div className="mt-16 flex flex-wrap items-center justify-between gap-6 border-t theme-border pt-8">
            ...
            <Link href={`/products/dude/${nextProduct.slug}/`} className="secondary-button">Continue exploring <ArrowRight className="h-4 w-4" /></Link>
          </div>
        </div>
      </section>
    </main>
